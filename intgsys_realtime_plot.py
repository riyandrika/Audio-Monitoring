...
class Intergrated_Sys:
    ...
    def record_audio(self, lock1, q, event, flag):
        ...
        # start Recording
        stream = self.audio.open(format=self.format, channels=self.channel,
                            rate=self.fs, input=True,
                            frames_per_buffer=self.chunk)
        
        while True:
            if flag.value == 1:
                data = stream.read(int(self.fs * duration))
                lock1.acquire()
                q.put(data)
                event.value = 1
                lock1.release()
...
if __name__ == "__main__":
    ...
    anim = animation.FuncAnimation(fig, control.animate, interval=1000, repeat=False)
    plt.show()
